...
boot_read_sectors(void)
{
    ...
    rc = boot_initialize_area(&boot_data, FLASH_AREA_IMAGE_0_S);
    if (rc != 0) {
        return BOOT_EFLASH;
    }
    
    rc = boot_initialize_area(&boot_data, FLASH_AREA_IMAGE_0_NS);
    if (rc != 0) {
        return BOOT_EFLASH;
    }

    rc = boot_initialize_area(&boot_data, FLASH_AREA_IMAGE_1_S);
    if (rc != 0) {
        return BOOT_EFLASH;
    }
    
    rc = boot_initialize_area(&boot_data, FLASH_AREA_IMAGE_1_NS);
    if (rc != 0) {
        return BOOT_EFLASH;
    }
...
boot_status_source(int part)
{
    ...
    if(part == PART_S)
		{
			rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_0_S, &state_slot0);
    	assert(rc == 0);

    	rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_SCRATCH_S, &state_scratch);
    	assert(rc == 0);		
		}
		else
		{
			rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_0_NS, &state_slot0);
    	assert(rc == 0);

    	rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_SCRATCH_NS, &state_scratch);
    	assert(rc == 0);				
		}
 ...
 */
static int
boot_previous_swap_type(int part)
{
    int post_swap_type;

    post_swap_type = boot_swap_type(part);
...
static int
boot_slots_compatible(int part)
{
    int img_start,max_sectors;
    size_t num_sectors_0,num_sectors_1;
    size_t size_0, size_1;
    size_t i;
    if(part == PART_S)
    {
    	img_start = 0;	
    	max_sectors = BOOT_MAX_IMG_SECTORS_S;
    }		    	
    else
    {
    	img_start = 1;	
    	max_sectors = BOOT_MAX_IMG_SECTORS_NS;	
    }
    	
		
		num_sectors_0 = boot_img_num_sectors(&boot_data, img_start);
    num_sectors_1 = boot_img_num_sectors(&boot_data, img_start+2);
    
    if (num_sectors_0 > max_sectors || num_sectors_1 > max_sectors) {
    ...
    for (i = 0; i < num_sectors_0; i++) {
        size_0 = boot_img_sector_size(&boot_data, img_start, i);
        size_1 = boot_img_sector_size(&boot_data, img_start+2, i);
        if (size_0 != size_1) {
...
static int
boot_read_status(struct boot_status *bs, int part)
{
    const struct flash_area *fap;
    int status_loc;
    int area_id;
    ...
    status_loc = boot_status_source(part);
    switch (status_loc) {
    case BOOT_STATUS_SOURCE_NONE:
        return 0;

    case BOOT_STATUS_SOURCE_SCRATCH:
        area_id = (part == PART_S) ? FLASH_AREA_IMAGE_SCRATCH_S:FLASH_AREA_IMAGE_SCRATCH_NS;
        break;

    case BOOT_STATUS_SOURCE_SLOT0:
        area_id = (part == PART_S) ? FLASH_AREA_IMAGE_0_S:FLASH_AREA_IMAGE_0_NS;
        break;
 ...
 */
int
boot_write_status(struct boot_status *bs,int part)
{
    const struct flash_area *fap = NULL;
    uint32_t off;
    int area_id;
 ...
 */
static int
boot_validated_swap_type(int part)
{
    int swap_type;
    int slot;

    swap_type = boot_swap_type(part);
    switch (swap_type) {
    case BOOT_SWAP_TYPE_TEST:
    case BOOT_SWAP_TYPE_PERM:
    case BOOT_SWAP_TYPE_REVERT:
        /* Boot loader wants to switch to slot 1. Ensure image is valid. */
        slot = ((part == PART_S)? 2:3);
        if ( boot_validate_slot(slot) != 0) {
...
static uint32_t
boot_copy_sz(int last_sector_idx, int *out_first_sector_idx, int part)
{
    size_t scratch_sz;
    uint32_t new_sz;
    uint32_t sz;
    int i;
    int slot = ((part == PART_S)?0:1);

    sz = 0;

    scratch_sz = boot_scratch_area_size(&boot_data,part);
    
    for (i = last_sector_idx; i >= 0; i--) {
        new_sz = sz + boot_img_sector_size(&boot_data, slot, i);
...
#ifdef TFM_SEGREGATE
static inline int
boot_status_init_by_id(int flash_area_id, const struct boot_status *bs,int part)
{
    const struct flash_area *fap;
    ...
    rc = boot_read_swap_state_by_id(slot, &swap_state);
    assert(rc == 0);
...
//#ifndef MCUBOOT_OVERWRITE_ONLY
static void
boot_swap_sectors(int idx, uint32_t sz, struct boot_status *bs,int part)
{
    const struct flash_area *fap;
    uint32_t copy_sz;
    uint32_t trailer_sz;
    ...
    size_t last_sector;
    int rc;
		int slot_0 = ((part == PART_S) ? FLASH_AREA_IMAGE_0_S : FLASH_AREA_IMAGE_0_NS);
    int slot_1 = ((part == PART_S) ? FLASH_AREA_IMAGE_1_S : FLASH_AREA_IMAGE_1_NS);
    int slot_s = ((part == PART_S) ? FLASH_AREA_IMAGE_SCRATCH_S : FLASH_AREA_IMAGE_SCRATCH_NS);

			
    /* Calculate offset from start of image area. */
    img_off = boot_img_sector_off(&boot_data, slot_0, idx);

    copy_sz = sz;
    trailer_sz = boot_slots_trailer_sz(BOOT_WRITE_SZ(&boot_data),part);
     ...
     */
    last_sector = boot_img_num_sectors(&boot_data, slot_0) - 1;
    if (img_off + sz > boot_img_sector_off(&boot_data, slot_0, last_sector)) {
        copy_sz -= trailer_sz;
    }
    ...
    if (bs->state == BOOT_STATUS_STATE_0) {
        rc = boot_erase_sector(slot_s, 0, sz);
        assert(rc == 0);

        rc = boot_copy_sector(slot_1, slot_s,
                              img_off, 0, copy_sz);
        assert(rc == 0);

        if (bs->idx == BOOT_STATUS_IDX_0) {
            if (bs->use_scratch) {
                boot_status_init_by_id(slot_s, bs,part);
            } else {
                /* Prepare the status area... here it is known that the
                 * last sector is not being used by the image data so it's
                 * safe to erase.
                 */
                rc = boot_erase_last_sector_by_id(slot_0);
                assert(rc == 0);

                boot_status_init_by_id(slot_0, bs,part);
            }
        }

        bs->state = BOOT_STATUS_STATE_1;
        rc = boot_write_status(bs,part);
        BOOT_STATUS_ASSERT(rc == 0);
    }

    if (bs->state == BOOT_STATUS_STATE_1) {
        rc = boot_erase_sector(slot_1, img_off, sz);
        assert(rc == 0);

        rc = boot_copy_sector(slot_0, slot_1,
        ...
        if (bs->idx == BOOT_STATUS_IDX_0 && !bs->use_scratch) {
             ...
             */
            rc = boot_erase_last_sector_by_id(slot_1);
            assert(rc == 0);
        }
    ...
    if (bs->state == BOOT_STATUS_STATE_2) {
        rc = boot_erase_sector(slot_0, img_off, sz);
        assert(rc == 0);

        /* NOTE: also copy trailer from scratch (has status info) */
        rc = boot_copy_sector(slot_s, slot_0,
                              0, img_off, copy_sz);
        assert(rc == 0);

        if (bs->use_scratch) {
            rc = flash_area_open(slot_s, &fap);
            assert(rc == 0);
            ...
            rc = boot_read_swap_state_by_id(slot_s,
                                            &swap_state);
            assert(rc == 0);
...
#ifdef TFM_SEGREGATE
static int
boot_copy_image(struct boot_status *bs,int part)
{
    size_t sect_count;
    size_t sect;
    int rc;
    ...
    sect_count = boot_img_num_sectors(&boot_data, slot);
    for (sect = 0; sect < sect_count; sect++) {
        this_size = boot_img_sector_size(&boot_data, slot, sect);
        rc = boot_erase_sector(slot+1,
                               size,
                               this_size);
        assert(rc == 0);
    ...
    rc = boot_copy_sector(slot+3, slot+1,
                          0, 0, size);
     ...
     */
    rc = boot_update_security_counter(slot, boot_img_hdr(&boot_data, slot+2));
    if (rc != 0) {
     ...
     */
    rc = boot_erase_sector(slot+3,
                           boot_img_sector_off(&boot_data, slot+2, 0),
                           boot_img_sector_size(&boot_data, slot+2, 0));
    assert(rc == 0);
    last_sector = boot_img_num_sectors(&boot_data, slot+2) - 1;
    rc = boot_erase_sector(slot+3,
                           boot_img_sector_off(&boot_data, slot+2, last_sector),
                           boot_img_sector_size(&boot_data, slot+2, last_sector));
    assert(rc == 0);
...
boot_copy_image(struct boot_status *bs,int part)
{
    ...
    if (bs->idx == BOOT_STATUS_IDX_0 && bs->state == BOOT_STATUS_STATE_0) {
        ...
        hdr = boot_img_hdr(&boot_data, slot);
        if (hdr->ih_magic == IMAGE_MAGIC) {
            rc = boot_read_image_size(slot, hdr, &copy_size);
            assert(rc == 0);
        }

        hdr = boot_img_hdr(&boot_data, slot+2);
        if (hdr->ih_magic == IMAGE_MAGIC) {
            rc = boot_read_image_size(slot+2, hdr, &size);
            assert(rc == 0);
        }
         ...
         * in the trailer...
         */
        rc = boot_read_swap_size(&bs->swap_size,part);
        assert(rc == 0);
    ...
    last_sector_idx = 0;
    while (1) {
        size += boot_img_sector_size(&boot_data,slot, last_sector_idx);
        if (size >= copy_size) {
            break;
        }
        last_sector_idx++;
    ...
    while (last_sector_idx >= 0) {
        sz = boot_copy_sz(last_sector_idx, &first_sector_idx,part);
        if (swap_idx >= (bs->idx - BOOT_STATUS_IDX_0)) {
            boot_swap_sectors(first_sector_idx, sz, bs,part);
        }
...
#ifdef TFM_SEGREGATE
static int
boot_swap_if_needed(int *out_swap_type,int part)
{
    ...
    if (bs.idx != BOOT_STATUS_IDX_0 || bs.state != BOOT_STATUS_STATE_0) {
         ...
         */
        swap_type = boot_previous_swap_type(part);
    } else {
        swap_type = boot_validated_swap_type(part);
        switch (swap_type) {
        case BOOT_SWAP_TYPE_TEST:
        case BOOT_SWAP_TYPE_PERM:
        case BOOT_SWAP_TYPE_REVERT:
...
{
		int swap_type_s,swap_type_ns;
    size_t slot,slot_s,slot_ns;
    int rc;
    int fa_id;
    bool reload_headers_s = false,reload_headers_ns = false;
    ...
    static boot_sector_t slot0_s_sectors[BOOT_MAX_IMG_SECTORS_S];
    static boot_sector_t slot1_s_sectors[BOOT_MAX_IMG_SECTORS_S];
    static boot_sector_t slot0_ns_sectors[BOOT_MAX_IMG_SECTORS_NS];
    static boot_sector_t slot1_ns_sectors[BOOT_MAX_IMG_SECTORS_NS];
    boot_data.imgs[0].sectors = slot0_s_sectors;
    boot_data.imgs[1].sectors = slot0_ns_sectors;
    boot_data.imgs[2].sectors = slot1_s_sectors;
    boot_data.imgs[3].sectors = slot1_ns_sectors;
    ...
    for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
		...
		rc = flash_area_open(FLASH_AREA_IMAGE_SCRATCH_S,
                         &BOOT_SCRATCH_AREA_S(&boot_data));
    assert(rc == 0);
    rc = flash_area_open(FLASH_AREA_IMAGE_SCRATCH_NS,
                         &BOOT_SCRATCH_AREA_NS(&boot_data));
    assert(rc == 0);

    /* Determine the sector layout of the image slots and scratch area. */
    rc = boot_read_sectors();
    if (rc != 0) {
        BOOT_LOG_WRN("Failed reading sectors; BOOT_MAX_IMG_SECTORS_S=%d - too small?",
                BOOT_MAX_IMG_SECTORS_S);
        goto out;
    }
    ...
    if (rc != 0) {
		...
		// CHAEK S PART SWAP TYPE
		if (boot_slots_compatible(PART_S)) {
        rc = boot_swap_if_needed(&swap_type_s,PART_S);
        assert(rc == 0);
        if (rc != 0) {
            goto out;
        }
         ...
         */
        if (swap_type_s == BOOT_SWAP_TYPE_REVERT ||
            swap_type_s == BOOT_SWAP_TYPE_FAIL) {
#ifndef MCUBOOT_OVERWRITE_ONLY
            rc = boot_set_image_ok(PART_S);
            if (rc != 0) {
                swap_type_s = BOOT_SWAP_TYPE_PANIC;
            }
#endif /* !MCUBOOT_OVERWRITE_ONLY */
        }
    } else {
        swap_type_s = BOOT_SWAP_TYPE_NONE;
    }
    // CHAEK NS PART SWAP TYPE
    if (boot_slots_compatible(PART_NS)) {
        rc = boot_swap_if_needed(&swap_type_ns,PART_NS);
        assert(rc == 0);
        if (rc != 0) {
            goto out;
        }
         ...
         * swap was finished to avoid a new revert.
         */
        if (swap_type_ns == BOOT_SWAP_TYPE_REVERT ||
            swap_type_ns == BOOT_SWAP_TYPE_FAIL) {
#ifndef MCUBOOT_OVERWRITE_ONLY
            rc = boot_set_image_ok(PART_NS);
            if (rc != 0) {
                swap_type_ns = BOOT_SWAP_TYPE_PANIC;
            }
#endif /* !MCUBOOT_OVERWRITE_ONLY */
        }
    } else {
        swap_type_ns = BOOT_SWAP_TYPE_NONE;
    }
    ...
    case BOOT_SWAP_TYPE_REVERT:
        slot_s = 2;
        reload_headers_s = true;
#ifndef MCUBOOT_OVERWRITE_ONLY

		if (swap_type_s == BOOT_SWAP_TYPE_PERM) {
             ...
             */
            rc = boot_update_security_counter(0, boot_img_hdr(&boot_data, 2));
            if (rc != 0) {
    ...
    case BOOT_SWAP_TYPE_REVERT:
        slot_ns = 3;
        reload_headers_ns = true;
#ifndef MCUBOOT_OVERWRITE_ONLY
        rc = boot_set_copy_done(PART_NS);
		if (swap_type_ns == BOOT_SWAP_TYPE_PERM) {
            /* Update the stored security counter with the new image's security
             * counter value (the one in the primary slot). Slot 0 holds the
             * new image at this point, but slot 1's image header must be
             * passed because the read image headers in the boot_data structure
             ...
             * counter must be increased right after the image upgrade.
             */
            rc = boot_update_security_counter(1, boot_img_hdr(&boot_data, 3));
            if (rc != 0) {
                BOOT_LOG_ERR("Security counter update failed after "
                             "image upgrade.");
                goto out;
    ...
    if (swap_type_s == BOOT_SWAP_TYPE_PANIC) {
        BOOT_LOG_ERR("panic!");
        assert(0);
    ...
    if (swap_type_ns == BOOT_SWAP_TYPE_PANIC) {
        BOOT_LOG_ERR("panic!");
        assert(0);
    ...
    if (boot_data.imgs[0].hdr.ih_magic != IMAGE_MAGIC) {
        BOOT_LOG_ERR("S part bad image magic 0x%lx", (unsigned long)boot_data.imgs[0].hdr.ih_magic);
        rc = BOOT_EBADIMAGE;
        goto out;
    }
    if (boot_data.imgs[1].hdr.ih_magic != IMAGE_MAGIC) {
        BOOT_LOG_ERR("NS part bad image magic 0x%lx", (unsigned long)boot_data.imgs[1].hdr.ih_magic);
        rc = BOOT_EBADIMAGE;
        goto out;
    }
     ...
     */
    if (swap_type_s == BOOT_SWAP_TYPE_NONE) {
	...
	if (swap_type_ns == BOOT_SWAP_TYPE_NONE) {
        rc = boot_update_security_counter(1, boot_img_hdr(&boot_data, 1));
        if (rc != 0) {
            BOOT_LOG_ERR("Security counter update failed after image "
                         "validation.");
            goto out;
    ...
    rsp->br_hdr = boot_img_hdr(&boot_data, slot_s);

    /* Save boot status to shared memory area */
    image_hdr = boot_img_hdr(&boot_data, slot_s);
    rc = boot_save_boot_status(SW_SPE,
    						   image_hdr,
                               BOOT_IMG_AREA(&boot_data, slot_s));
    if (rc) {
        BOOT_LOG_ERR("Failed to add S part data to shared area");
    }
    image_hdr = boot_img_hdr(&boot_data, slot_ns);
    rc = boot_save_boot_status(SW_NSPE,
    						   image_hdr,
                               BOOT_IMG_AREA(&boot_data, slot_ns));
    if (rc) {
        BOOT_LOG_ERR("Failed to add NS part data to shared area");
    }

 out:
    flash_area_close(BOOT_SCRATCH_AREA_S(&boot_data));
    flash_area_close(BOOT_SCRATCH_AREA_NS(&boot_data));
    for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
